...
/**
 * Mirrors auth.users.id when using Supabase Auth. `id` must equal Supabase user UUID.
 * Add FK to auth.users in Supabase SQL migrations (not expressible in all Drizzle versions).
 */
...
export const organizationMembers = pgTable(
  "organization_members",
  {
    ...
    /** Legacy NextAuth / credentials users.id */
    userId: uuid("user_id").references(() => users.id, { onDelete: "cascade" }),
    /** Supabase Auth user id (profiles.id / auth.users.id) */
    authUserId: uuid("auth_user_id").references(() => profiles.id, { onDelete: "cascade" }),
  ...
  (t) => [
    ...
    uniqueIndex("org_members_auth_org_unique").on(t.authUserId, t.organizationId),
  ]
);
...
export const clients = pgTable(
  "clients",
  {
    ...
    organizationId: uuid("organization_id")
      ...
      .references(() => organizations.id, { onDelete: "cascade" }),
    name: text("name").notNull(),
    /** Default for widget (when visitor does not send replyLanguage) and WhatsApp. */
    replyLanguage: varchar("reply_language", { length: 32 }).notNull().default("english"),
...
/**
 * Admin-allowlisted URLs fetched server-side into RAG (document_chunks).
 * When `refresh_on_new_conversation` is true, content is re-fetched at the start of each new conversation thread.
 */
export const clientWebSources = pgTable(
  "client_web_sources",
  {
    id: uuid("id").defaultRandom().primaryKey(),
    ...
    refreshOnNewConversation: boolean("refresh_on_new_conversation").notNull().default(true),
    createdAt: timestamp("created_at", { withTimezone: true }).defaultNow().notNull(),
    updatedAt: timestamp("updated_at", { withTimezone: true }).defaultNow().notNull(),
  },
  (t) => [
    index("cws_client_idx").on(t.clientId),
    uniqueIndex("cws_client_url_unique").on(t.clientId, t.url),
  ]
);

/**
 * Sandboxed merchant portal: one row grants a login access to exactly one client (business) in an org.
 * Use `user_id` for NextAuth credentials users, or `auth_user_id` for Supabase Auth (not both).
 */
export const clientPortalAccess = pgTable(
  ...
  (t) => [
    index("cpa_org_idx").on(t.organizationId),
    index("cpa_client_idx").on(t.clientId),
    index("cpa_user_idx").on(t.userId),
    index("cpa_auth_idx").on(t.authUserId),
  ]
);
...
export const clientsRelations = relations(clients, ({ one, many }) => ({
  ...
  webSources: many(clientWebSources),
}));

export const clientWebSourcesRelations = relations(clientWebSources, ({ one }) => ({
  client: one(clients, {
    fields: [clientWebSources.clientId],

...
export async function POST(
  ...
  const replyLanguage = typeof body.replyLanguage === "string" ? body.replyLanguage.trim() : undefined;
  ...
  if (isMockData()) {
    return NextResponse.json(
      { ...mockWidgetReply(message), correlationId },
      { headers: { ...corsHeaders, "X-Correlation-Id": correlationId } }
    );
  }

  const edgeUrl = process.env.WIDGET_CHAT_EDGE_URL?.trim();
  if (edgeUrl) {
    const target = edgeUrl.includes("{{publicId}}")
      ? edgeUrl.replace(/\{\{publicId\}\}/g, publicId)
      : edgeUrl.replace(/\/$/, "");
  ...
  const result = await handleCustomerMessage({
    ...
    replyLanguage,
    correlationId,
    rawPayload: { visitorId, replyLanguage },
  });

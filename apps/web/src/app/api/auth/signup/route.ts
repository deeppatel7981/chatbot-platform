  ...
  try {
    ...
    if (message.includes("DATABASE_URL is not set")) {
      const error = process.env.VERCEL
        ? "Database is not configured. In Vercel: Settings → Environment Variables → add DATABASE_URL (Postgres URI) for the same environment as this deployment (e.g. Production), save, then Redeploy. You can also use POSTGRES_URL or SUPABASE_DATABASE_URL."
    ...
    if (code === "42P01" || (message.includes("does not exist") && message.includes("relation"))) {
      return NextResponse.json(
        {
          error: "Database tables are missing. From the repo run: npm run db:migrate (DATABASE_URL in .env.local), or apply SQL from drizzle/ in order.",
          details,
        },
        { status: 500 }
      );
    ...
    if (code === "28P01" || message.toLowerCase().includes("password authentication failed")) {
      return NextResponse.json(
        {
          error: "Database login failed. Check the password in DATABASE_URL in Vercel (Supabase → Database → reset password, then update the env var and redeploy).",
        },
        { status: 503 }
      );
    }
    ...
    const hint =
      "If this database is new, run `npm run db:migrate` from the repo with DATABASE_URL set to your Supabase URI. After schema edits, run `npm run db:generate` then commit `drizzle/` and migrate again. Check Vercel → Logs for the real error.";

    return NextResponse.json(
      { error: "Could not create account. Try again.", hint, details },
      { status: 500 }
    );
  }
}

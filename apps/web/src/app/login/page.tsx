...
const LoginPageContent = () => {
  ...
  const registeredBanner = searchParams.get("registered") === "1";
  ...
  const handleLogin = async (e: React.FormEvent<HTMLFormElement>) => {
    ...
    if (sb) {
      ...
      router.push(next && next.startsWith("/") ? next : "/app/overview");
      router.refresh();
      return;
    }
    ...
    if (res?.ok) {
      ...
      if (mockMode) {
        ...
        setError(
          healthHint ||
            "Wrong email/password, or no user in the database. Set DATABASE_URL, run npm run db:migrate && npm run db:seed, then admin@example.com / admin"
        );
      }
    } else {
          ...
          >
            ...
            <p className="mt-4 text-center text-xs leading-relaxed text-zinc-500 dark:text-zinc-500">
              {mockMode === true ? (
                ...
                <>
                  Migrate: <code className="rounded bg-zinc-100 px-1 dark:bg-zinc-800">npm run db:migrate</code>{" "}

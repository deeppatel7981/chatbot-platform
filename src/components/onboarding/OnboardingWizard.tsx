...
export default function OnboardingWizard() {
  ...
  useEffect(() => {
    if (session?.user?.email && !state.notifyEmail) {
      setState((s) => ({ ...s, notifyEmail: session.user?.email ?? "" }));
    }
  }, [session?.user?.email, state.notifyEmail]);
        ...
        <div className="rounded-2xl border border-zinc-200 bg-white p-6 shadow-sm dark:border-zinc-800 dark:bg-zinc-950 sm:p-8">
          ...
          {step === 6 && (
            <div className="space-y-4">
              ...
              <div className="space-y-1.5">
                ...
                <Input type="email" value={state.notifyEmail} onChange={(e) => update({ notifyEmail: e.target.value })} />
              </div>
              <div className="space-y-1.5">
          ...
          {step === 7 && (
            <div>
              ...
              <p className="mt-1 text-sm text-zinc-600 dark:text-zinc-400">Try the visitor experience — then finish to create your first client in the workspace.</p>

...
export default function ClientTable({ refreshKey }: Props) {
  ...
  if (clients.length === 0)
    return (
      <div className="rounded-2xl border border-dashed border-zinc-300 bg-zinc-50/50 px-6 py-12 text-center text-sm text-zinc-600 dark:border-zinc-700 dark:bg-zinc-900/40 dark:text-zinc-400">
        {error ? (
          <span className="text-red-600 dark:text-red-400">Error: {error}</span>
        ) : (
          "No businesses yet — add one with “New client”."
        )}
      </div>
    );

...
function statusPill(status: string) {
  ...
  return (
    <span className={`inline-flex rounded-full px-2.5 py-0.5 text-xs font-medium capitalize ${cls}`}>{status}</span>
  );
}
...
export default function ChatLogsPage() {
  ...
  return (
    <div>
      <PageIntro
        ...
        description={
          <p className="max-w-2xl">
            Triage chats from your site and WhatsApp in one place.{" "}
            <strong className="font-medium text-zinc-800 dark:text-zinc-200">Handoff</strong> means a teammate should step
            in. <strong className="font-medium text-zinc-800 dark:text-zinc-200">Confidence</strong> is how sure the last
            reply was — low confidence usually needs a quick human review.
          </p>
        }
      />
      ...
      <div className="mb-4 flex flex-wrap items-center gap-3 rounded-2xl border border-zinc-200/80 bg-white px-3 py-2 shadow-sm dark:border-zinc-800 dark:bg-zinc-900">
        <label className="relative min-w-[200px] flex-1">
          ...
          <input
            ...
            placeholder="Search business, channel, or ID…"
            className="w-full rounded-xl border border-zinc-200 bg-zinc-50/80 px-3 py-2 pl-9 text-sm text-zinc-900 placeholder:text-zinc-400 focus:border-violet-500/50 focus:outline-none focus:ring-2 focus:ring-violet-500/20 dark:border-zinc-700 dark:bg-zinc-950 dark:text-zinc-100"
          />
      ...
      <div className="mb-3 flex flex-wrap gap-1.5" role="tablist" aria-label="Filter conversations">
        {tabs.map((t) => (
          <button
            key={t.id}
            type="button"
      ...
      <div className="overflow-x-auto rounded-2xl border border-zinc-200/80 bg-white shadow-dashboard-card dark:border-zinc-800 dark:bg-zinc-900">
        <table className="min-w-full text-sm">
          <thead>
            <tr className="border-b border-zinc-200 text-left dark:border-zinc-800">
              <th className="px-4 py-3.5 text-xs font-semibold uppercase tracking-wide text-zinc-500 dark:text-zinc-400">Business</th>
              <th className="px-4 py-3.5 text-xs font-semibold uppercase tracking-wide text-zinc-500 dark:text-zinc-400">Channel</th>
              <th className="px-4 py-3.5 text-xs font-semibold uppercase tracking-wide text-zinc-500 dark:text-zinc-400">Status</th>
              <th className="px-4 py-3.5 text-xs font-semibold uppercase tracking-wide text-zinc-500 dark:text-zinc-400">Handoff</th>
              <th className="px-4 py-3.5 text-xs font-semibold uppercase tracking-wide text-zinc-500 dark:text-zinc-400">Confidence</th>
              <th className="px-4 py-3.5 text-xs font-semibold uppercase tracking-wide text-zinc-500 dark:text-zinc-400">Updated</th>
        ...
        {rows.length === 0 && !error && (
          <div className="px-4 py-12 text-center">
            ...
            <p className="mx-auto mt-2 max-w-md text-sm text-zinc-500 dark:text-zinc-400">
              When shoppers use the widget or WhatsApp, threads appear here. Set up channels under{" "}
              <Link
                href="/dashboard/clients"
                className="font-medium text-violet-700 underline decoration-violet-300 underline-offset-2 dark:text-violet-400"
              >
                Clients
              </Link>{" "}
              and{" "}
